...
写到这里，你已经走完了本书的路线图章节。前面十九章帮你搭起了从行业认知、基础知识、Agent 架构，到面试与实操的完整骨架；这一章的任务，是把“知道该学什么”进一步变成“明天就能开始执行的路线图”。如果说前面的章节是在帮你建立地图，那么这一章更像是为你准备背包、补给和里程碑。
...
| Tooling-chain 冲刺 | 本书第 7-12、16、18、21-22 章 + 一个自定义 tool registry / MCP server 小项目 |
...
### 9.1 偏 Tooling Chain 岗位的一周压缩读法

如果目标岗位明显强调 Agent 工具链、MCP、RAG、工程化、评估或落地经验，不建议按章节顺序从头重读。更高效的顺序是：

| 天数 | 章节 | 产出 |
|------|------|------|
| Day 1 | 第 8、9 章 | 画出 Agent + Tool Registry + Runtime 总图 |
| Day 2 | 第 12 章 | 讲清 Function Calling、MCP、A2A、内部 API 的边界 |
| Day 3 | 第 7、10 章 | 准备 RAG 故障排查与 memory 设计答案 |
| Day 4 | 第 11、15 章 | 对比 ReAct、Graph、多 Agent 与主流框架选型 |
| Day 5 | 第 16 章 | 准备部署、监控、成本、安全和回滚答案 |
| Day 6 | 第 18、21 章 | 练系统设计题与 Agentic Engineering 项目表达 |
| Day 7 | 第 22 章 | 用 tooling-chain 主线做总复盘和模拟面试 |

这个顺序来自公开 AI Agent / 大模型应用岗位的常见追问：工具调用、RAG、MCP、评估、可观测性、生产部署和真实项目经验。不要把它当成背题表，而要把每一天都变成一个可讲述的工程产物。
...
- 偏 tooling-chain 的面试应优先串起工具调用、MCP/RAG、eval、observability、安全和部署，而不是平均复习所有概念。
