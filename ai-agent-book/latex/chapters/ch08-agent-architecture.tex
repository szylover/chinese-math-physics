...
很多工程师第一次做 Agent，实际上只是把一个大语言模型（Large Language Model, LLM）包装成``问一句、答一句''的聊天接口。这样的系统当然有价值，但它离真正的 Agent 还有明显距离。判断一个系统是不是 Agent，关键不在 UI 长得像不像聊天框，而在它是否具备\textbf{自主性、工具使用能力、规划能力、状态维持能力以及基于反馈继续迭代的闭环执行能力}。这一章我们把这些能力拆开，讲清楚主流架构模式，并从零实现一个可以运行的 ReAct Agent。
...
\hline
类型 & 输入 & 输出 & 是否自主决策 & 是否调用工具 & 是否有循环 \\ \hline
单次 LLM 调用 & Prompt & 文本 & 否 & 否 & 否 \\ \hline
带函数调用的问答 & Prompt + Tools & 文本/工具结果 & 弱 & 是 & 弱 \\ \hline
工作流式 Agent & Goal & 多步执行结果 & 中 & 是 & 是 \\ \hline
通用 Agent & Goal + 环境状态 & 多轮计划、执行、反思结果 & 强 & 是 & 是 \\ \hline
\end{longtable}
\endgroup

一个`\texttt{简单 LLM 调用''通常只有两步：拼 Prompt，拿输出。它可以回答}\texttt{北京今天天气如何''，但不能自己去查天气 API、不能在失败后自动重试、不能分解}`帮我比较未来三天上海和深圳的出行建议''这种任务。
...
\item \textbf{Autonomy（自主性）}：接到目标后，不需要每一步都由用户显式指挥。
\item \textbf{Tool Use（工具使用）}：会调用搜索、数据库、代码执行、文件系统、浏览器等外部能力。
\item \textbf{Planning（规划）}：面对复杂目标时，会分解任务、排序步骤、决定先做什么再做什么。
\item \textbf{Memory（记忆）}：能保留上下文、追踪中间状态，甚至跨会话保留事实与经验。
\end{enumerate}

如果你把`\texttt{工具调用''理解成 API 编排，把}\texttt{规划''理解成任务调度，把}`记忆''理解成状态管理，那么 Agent 架构其实就是\textbf{传统软件工程 + 概率模型决策层}。这也是为什么后端、平台、基础设施工程师转做 Agent 非常有优势。
...
\item \textbf{Perceive（感知）}：读取用户目标、历史上下文、工具返回值、外部环境状态。
\item \textbf{Reason（推理）}：模型决定当前最合理的下一步，是回答、调用工具、修改计划还是结束。
\item \textbf{Act（行动）}：真正执行动作，比如调用 HTTP API、写文件、执行 SQL。
\item \textbf{Observe（观察）}：收集动作结果，并写回工作记忆（working memory）。
\end{itemize}
...
\item Perceive = 请求解析层
\item Reason = LLM 决策层
\item Act = Tool Runtime
\item Observe = State Reducer
\end{itemize}

很多面试题都会问：``Agent 为什么比 Workflow 更灵活？''答案就在这里：Workflow 的路径大多是预先编码的，而 Agent 的路径是在 \textbf{Reason 阶段动态生成} 的。


\section{8.3 核心架构模式一：ReAct（Reason + Act）}

ReAct 是目前最经典、最容易落地的 Agent 模式。它把`\texttt{思考''和}`行动''交替展开，让模型在每一步先判断需要什么，再调用对应工具。


\subsection{8.3.1 ReAct 的核心格式}
