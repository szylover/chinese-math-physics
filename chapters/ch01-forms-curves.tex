...
本章同时引入模形式理论的三个核心对象：\textbf{模形式}、\textbf{椭圆曲线}
和\textbf{模曲线}。这三者之间的深刻联系贯穿全书，并最终在费马大定理的
证明中扮演关键角色。

我们将看到：模形式是上半平面上具有特殊对称性的全纯函数；
椭圆曲线是亏格为 $1$ 的代数曲线，同构于复环面 $\C/\Lambda$；
而模曲线则是参数化椭圆曲线（附加结构）的模空间。
...
\textbf{上半平面}（upper half-plane）定义为
\[
...
\[
  ...
  \right\}.
\]
它通过\textbf{莫比乌斯变换}（Möbius transformation）作用在 $\HH$ 上：
\[
...
\begin{remark}[为什么用 $\SL_2$ 而非 $\GL_2$]
\label{rem:why-sl2}
...
\begin{remark}[$-I$ 的作用与 $\PSL_2(\Z)$]
\label{rem:psl2}
$-I = \begin{pmatrix} -1 & 0 \\ 0 & -1 \end{pmatrix}$ 作用平凡：
$(-I)(\tau) = \frac{-\tau}{-1} = \tau$。因此实际上是
$\PSL_2(\Z) = \SL_2(\Z)/\{\pm I\}$ 在 $\HH$ 上\emph{忠实}地作用。
\end{remark}

\begin{remark}[群作用的基本概念]
\label{rem:group-action}
回顾群作用的定义。群 $G$ \textbf{作用在}（acts on）集合 $X$ 上，
是指一个映射 $G \times X \to X$（记 $(g, x) \mapsto g \cdot x$），满足：
\begin{enumerate}
  \item \textbf{单位元}：$e \cdot x = x$，对所有 $x \in X$；
  \item \textbf{相容性}：$(g_1 g_2) \cdot x = g_1 \cdot (g_2 \cdot x)$，
    对所有 $g_1, g_2 \in G$，$x \in X$。
\end{enumerate}
相关概念：
\begin{itemize}
  \item \textbf{轨道}（orbit）：$G \cdot x = \{g \cdot x \mid g \in G\}$；
  \item \textbf{稳定子}（stabilizer）：
    $G_x = \{g \in G \mid g \cdot x = x\}$，是 $G$ 的子群；
  \item \textbf{商空间}（quotient）：
    $G \backslash X = \{G \cdot x \mid x \in X\}$，所有轨道的集合。
\end{itemize}
轨道-稳定子定理：$\#(G \cdot x) = [G : G_x]$（$G$ 有限时）。
后续的模曲线 $\SL_2(\Z) \backslash \HH$ 就是这里的商空间。
\end{remark}
...
两个特殊点 $i$ 和 $\rho = e^{2\pi i/3} = (-1+i\sqrt{3})/2$ 是
$\SL_2(\Z)$ 的\textbf{椭圆不动点}：$S(i) = i$，$(ST)(ρ) = ρ$。
\end{remark}
...
\]
$\SL_2(\Z)$ 的作用自然延拓到 $\mathbb{P}^1(\Q)$ 上：
$\gamma(\infty) = a/c$，$\gamma(-d/c) = \infty$。
