...
\begin{enumerate}[label=\textbf{A.\arabic*.}, leftmargin=*, itemsep=1em]
  ...
  \[
    ...
    \Id_Y\circ f=f.
  \]
  其余复合都因源与靶不匹配而无意义，例如 $\Id_X\circ f$ 与 $f\circ \Id_Y$ 都不能定义，也不存在从 $Y$ 到 $X$ 的非恒等态射，所以 $f\circ f$ 也无意义。恒等律已由上式给出。至于结合律，任何可作复合的三重态射都只能含有恒等态射，例如
  \[
    (\Id_Y\circ f)\circ \Id_X=f\circ \Id_X=f,
    \qquad
    \Id_Y\circ (f\circ \Id_X)=\Id_Y\circ f=f,
  \]
  其余情形同理。因此这些数据确实定义了一个范畴。

  \item \textbf{解答.} 设 $e,e'\colon X\to X$ 都是对象 $X$ 的恒等态射。由于 $e$ 是左恒等元，$e\circ e'=e'$；由于 $e'$ 是右恒等元，$e\circ e'=e$。故 $e=e'$，从而恒等态射唯一。

  \item \textbf{解答.} 设 $g,h\colon Y\to X$ 都是 $f\colon X\to Y$ 的逆态射，即
  \[
    g\circ f=\Id_X,
    \qquad f\circ g=\Id_Y,
    \qquad
  ...
  \item \textbf{解答.}
  \begin{enumerate}[label=(\alph*), leftmargin=2.5em]
    \item 这是函子，而且是协变函子。对线性映射 $f\colon V\to W$，把它看成底层集合之间的映射即可。显然保持复合与恒等。
    \item 这是函子，而且是逆变函子。对 $f\colon V\to W$ 定义
    \[
      f^{\vee}\colon W^{\vee}\to V^{\vee},
      \qquad \lambda\mapsto \lambda\circ f.
    \]
    则 $(g\circ f)^{\vee}=f^{\vee}\circ g^{\vee}$，且 $(\Id_V)^{\vee}=\Id_{V^{\vee}}$。
    \item 这是协变函子。对 $R$-模同态 $f\colon M\to N$，因 $f(2M)=2f(M)\subseteq 2N$，故可定义诱导同态
    \[
      \overline f\colon M/2M\to N/2N,
      \qquad m+2M\mapsto f(m)+2N.
    \]
    良定义性与函子公理都直接成立。
    \item 严格地说，这样写一般不给出从 $\mathbf{Top}$ 到 $\mathbf{Grp}$ 的函子，因为基本群需要选基点；若不选基点，只能得到“在道路连通情形下到同构为止”的对象。若改成带基点拓扑空间范畴 $\mathbf{Top}_*$，则
    \[
      \pi_1\colon \mathbf{Top}_*\to \mathbf{Grp}
    \]
    是协变函子：带基点连续映射诱导群同态，且与复合、恒等相容。
  \end{enumerate}

  \item \textbf{解答.} 固定 $A\in \mathcal C$。
  对协变 Hom 函子 $\Hom_{\mathcal C}(A,-)$，若 $f\colon X\to Y$，则
  \[
    \Hom_{\mathcal C}(A,f)\colon \Hom_{\mathcal C}(A,X)\to \Hom_{\mathcal C}(A,Y),
    \qquad u\mapsto f\circ u.
  \]
  若 $X\xrightarrow{f}Y\xrightarrow{g}Z$，则对任意 $u\colon A\to X$，
  \[
    \Hom(A,g\circ f)(u)=(g\circ f)\circ u=g\circ (f\circ u)
    =\Hom(A,g)(\Hom(A,f)(u)),
  \]
  且 $\Hom(A,\Id_X)(u)=\Id_X\circ u=u$，故它是协变函子。

  对逆变 Hom 函子 $\Hom_{\mathcal C}(-,A)$，若 $f\colon X\to Y$，则
  \[
    \Hom_{\mathcal C}(f,A)\colon \Hom_{\mathcal C}(Y,A)\to \Hom_{\mathcal C}(X,A),
    \qquad v\mapsto v\circ f.
  \]
  对 $X\xrightarrow{f}Y\xrightarrow{g}Z$ 与任意 $w\colon Z\to A$，有
  \[
    \Hom(g\circ f,A)(w)=w\circ (g\circ f)=(w\circ g)\circ f
    =\Hom(f,A)(\Hom(g,A)(w)),
  \]
  即 $\Hom(g\circ f,A)=\Hom(f,A)\circ \Hom(g,A)$，且 $\Hom(\Id_X,A)=\Id$。故它是逆变函子。

  \item \textbf{解答.} 不存在自然变换 $\mathcal P\Rightarrow \Id$。事实上，对空集 $\varnothing$，若有自然变换 $\eta$，则其在 $\varnothing$ 上的分量应为函数
  \[
    \eta_{\varnothing}\colon \mathcal P(\varnothing)=\{\varnothing\}\to \varnothing,
  \]
  但从单元素集到空集不存在函数，矛盾。

  另一方面，存在自然变换 $\Id\Rightarrow \mathcal P$：对每个集合 $X$ 定义
  \[
    \iota_X\colon X\to \mathcal P(X),
    \qquad x\mapsto \{x\}.
  \]
  若 $f\colon X\to Y$，则对任意 $x\in X$，
  \[
    \mathcal P(f)(\iota_X(x))=\mathcal P(f)(\{x\})=\{f(x)\}=\iota_Y(f(x)).
  \]
  故交换图
  \[
    \mathcal P(f)\circ \iota_X=\iota_Y\circ f
  \]
  成立，所以 $\iota$ 是自然变换。

  \item \textbf{解答.} 记
  \[
    T\colon \Hom_{\F}(V,W)\to \Hom_{\F}(W^{\vee},V^{\vee}),
    \qquad f\mapsto f^{\vee}.
  \]
  它显然是线性的。先证单射。若 $f^{\vee}=0$，则对任意 $\lambda\in W^{\vee}$，$\lambda\circ f=0$。若存在 $v\in V$ 使 $f(v)\neq 0$，则可取 $\lambda\in W^{\vee}$ 满足 $\lambda(f(v))\neq 0$，于是
  \[
    0=f^{\vee}(\lambda)(v)=\lambda(f(v)),
  \]
  矛盾。故 $f=0$，从而 $T$ 单射。

  再比较维数：若 $\dim V=r$，$\dim W=s$，则
  \[
    \dim \Hom_{\F}(V,W)=rs,
    \qquad
    \dim \Hom_{\F}(W^{\vee},V^{\vee})=(\dim W^{\vee})(\dim V^{\vee})=sr.
  \]
  两端维数相同，故单射 $T$ 必为同构。
  ...
  \]
  短正合，且存在 $s\colon C\to B$ 使 $v\circ s=\Id_C$。定义
  \[
    \Phi\colon A\oplus C\to B,
    \qquad \Phi(a,c)=u(a)+s(c).
  \]
  它是同态。先证单射：若 $\Phi(a,c)=0$，对两边施行 $v$ 得
  \[
    0=v(u(a))+v(s(c))=0+c=c,
  \]
  故 $c=0$。于是 $u(a)=0$，而 $u$ 单射，所以 $a=0$。

  再证满射：任取 $b\in B$，令 $c=v(b)$。则
  \[
    v\bigl(b-s(c)\bigr)=v(b)-v(s(c))=c-c=0,
  \]
  所以 $b-s(c)\in \Ker(v)=\Ima(u)$。于是存在 $a\in A$ 使 $b-s(c)=u(a)$，即
  \[
    b=u(a)+s(c)=\Phi(a,c).
  \]
  故 $\Phi$ 为同构，从而 $B\cong A\oplus C$。

  \item \textbf{解答.} 设 $\pi\colon \Z\to \Z/2\Z$ 为自然投影。则给定序列是
  \[
    0\to \Z \xrightarrow{\times 2} \Z \xrightarrow{\pi} \Z/2\Z \to 0.
  \]
  映射 $\times 2$ 单射，因为 $2n=0$ 只能推出 $n=0$。映射 $\pi$ 满射，这是商映射的定义。最后
  \[
    \Ker(\pi)=\{n\in \Z: n\equiv 0\pmod 2\}=2\Z=\Ima(\times 2).
  \]
  因而该序列在每一处都正合，所以它是短正合序列。

  \item \textbf{解答.} 对每个左 $R$-模 $N$，赋值为阿贝尔群 $\Hom_R(M,N)$。若 $f\colon N\to N'$ 为 $R$-模同态，定义
  \[
    \Hom_R(M,f)\colon \Hom_R(M,N)\to \Hom_R(M,N'),
    \qquad \varphi\mapsto f\circ \varphi.
  \]
  因 $f$ 与 $\varphi$ 都是 $R$-线性的，$f\circ \varphi$ 仍是 $R$-线性的，所以此映射良定义。它还是群同态，因为
  \[
    \Hom_R(M,f)(\varphi+\psi)=f\circ (\varphi+\psi)=f\circ \varphi+f\circ \psi.
  \]
  若 $N\xrightarrow{f}N'\xrightarrow{g}N''$，则
  \[
    \Hom_R(M,g\circ f)(\varphi)=(g\circ f)\circ \varphi
    =g\circ (f\circ \varphi)
    =\Hom_R(M,g)(\Hom_R(M,f)(\varphi)),
  \]
  且
  \[
    \Hom_R(M,\Id_N)(\varphi)=\Id_N\circ \varphi=\varphi.
  \]
  因而 $N\mapsto \Hom_R(M,N)$ 连同后复合确实给出协变函子 $R\text{-}\mathbf{Mod}\to \mathbf{Ab}$。

  \item \textbf{解答.} 定义
  \[
    \Phi\colon \mathrm{Nat}(\Hom_{\mathcal C}(A,-),F)\to F(A),
    \qquad \Phi(\eta)=\eta_A(\Id_A).
  \]
  下证其为双射。

  对任意 $x\in F(A)$，定义一族映射
  \[
    \eta_X^x\colon \Hom_{\mathcal C}(A,X)\to F(X),
    \qquad f\mapsto F(f)(x).
  \]
  若 $g\colon X\to Y$，则对 $f\colon A\to X$，
  \[
    F(g)(\eta_X^x(f))=F(g)(F(f)(x))=F(g\circ f)(x)=\eta_Y^x(g\circ f),
  \]
  所以 $\eta^x\colon \Hom_{\mathcal C}(A,-)\Rightarrow F$ 是自然变换。

  反过来，给定自然变换 $\eta$，令 $x=\eta_A(\Id_A)$。对任意 $f\colon A\to X$，由自然性，交换图
  \[
  \begin{tikzcd}
  \Hom(A,A) \arrow[r,"{\eta_A}"] \arrow[d,"{\Hom(A,f)}"'] & F(A) \arrow[d,"{F(f)}"] \\
  \Hom(A,X) \arrow[r,"{\eta_X}"'] & F(X)
  \end{tikzcd}
  \]
  给出
  \[
    \eta_X(f)=\eta_X(\Hom(A,f)(\Id_A))=F(f)(\eta_A(\Id_A))=F(f)(x)=\eta_X^x(f).
  \]
  故 $\eta=\eta^x$。另一方面，
  \[
    \Phi(\eta^x)=\eta_A^x(\Id_A)=F(\Id_A)(x)=x.
  \]
  因而 $\Phi$ 既单射又满射，所以是双射。这就是 Yoneda 引理在此情形下的表述。

  \item \textbf{解答.} 设
  \[
    \Theta\colon \Hom_{\mathcal C}(A,-)\xRightarrow{\sim}\Hom_{\mathcal C}(B,-)
  \]
  为自然同构。由上一题（取 $F=\Hom_{\mathcal C}(B,-)$）可知，$\Theta$ 对应于唯一态射
  \[
    u\in \Hom_{\mathcal C}(B,A),
    \qquad u=\Theta_A(\Id_A).
  \]
  同理，$\Theta^{-1}$ 对应于唯一态射
  \[
    v\in \Hom_{\mathcal C}(A,B),
    \qquad v=\Theta^{-1}_B(\Id_B).
  \]
  再由 Yoneda 引理，恒等自然变换 $\Id_{\Hom(A,-)}$ 对应于 $\Id_A$，而复合自然变换 $\Theta^{-1}\circ \Theta$ 对应于 $u\circ v$；由于二者相等，得 $u\circ v=\Id_A$。同理，$\Theta\circ \Theta^{-1}=\Id_{\Hom(B,-)}$ 导致 $v\circ u=\Id_B$。故 $u$ 与 $v$ 互逆，从而 $A\cong B$。
  ...
  \end{tikzcd}
  \]
  其中 $\alpha'$ 与 $\alpha''$ 为同构。先证 $\alpha$ 单射。若 $\alpha(a)=0$，则
  \[
    \alpha''(p(a))=q(\alpha(a))=0.
  \]
  因 $\alpha''$ 单射，得 $p(a)=0$，故 $a\in \Ker(p)=\Ima(i)$。于是存在 $a'\in A'$ 使 $a=i(a')$。再由交换性，
  \[
    0=\alpha(a)=\alpha(i(a'))=j(\alpha'(a')).
  \]
  由于 $j$ 单射，$\alpha'(a')=0$；而 $\alpha'$ 为同构，故 $a'=0$，从而 $a=0$。

  再证 $\alpha$ 满射。任取 $b\in B$。设 $b''=q(b)\in B''$。因 $\alpha''$ 满射，存在 $a''\in A''$ 使 $\alpha''(a'')=b''$。又因 $p$ 满射，存在 $a\in A$ 使 $p(a)=a''$。于是
  \[
    q(b-\alpha(a))=q(b)-q(\alpha(a))=b''-\alpha''(p(a))=0,
  \]
  故 $b-\alpha(a)\in \Ker(q)=\Ima(j)$。于是存在 $b'\in B'$ 使 $b-\alpha(a)=j(b')$。由 $\alpha'$ 满射，存在 $a'\in A'$ 使 $\alpha'(a')=b'$。于是
  \[
    b=\alpha(a)+j(\alpha'(a'))=\alpha(a)+\alpha(i(a'))=\alpha(a+i(a')).
  \]
  所以 $\alpha$ 满射。综上，$\alpha$ 是同构。

  \item \textbf{解答.} 对短正合序列
  \[
    0\to \Z \xrightarrow{\times n} \Z \to \Z/n\Z \to 0
  \]
  施行逆变函子 $\Hom_{\Z}(-,\Z)$，得到左正合序列
  \[
    0\to \Hom_{\Z}(\Z/n\Z,\Z)\to \Hom_{\Z}(\Z,\Z)
    \xrightarrow{(\times n)^*} \Hom_{\Z}(\Z,\Z).
  \]
  由 $\Hom_{\Z}(\Z,\Z)\cong \Z$，且预复合 $ (\times n)^*$ 在此识别下正是乘以 $n$ 的映射，所以该序列化为
  \[
    0\to \Hom_{\Z}(\Z/n\Z,\Z)\to \Z \xrightarrow{\times n} \Z.
  \]
  又因为从有限群 $\Z/n\Z$ 到无挠群 $\Z$ 的群同态只能是零映射，故
  \[
    \Hom_{\Z}(\Z/n\Z,\Z)=0.
  \]
  因而得到
  \[
    0\to 0\to \Z \xrightarrow{\times n} \Z.
  \]
  当 $n>1$ 时，乘以 $n$ 的映射不是满射，所以若再要求右正合，就会错误地得到
  \[
    \Z \xrightarrow{\times n} \Z \to 0
  \]
  在最右端正合；这显然不成立。故 $\Hom_{\Z}(-,\Z)$ 一般不是右正合的。

  \item \textbf{解答.} 设 $q\colon B\to B/u(A)$ 为商映射。因为 $q\circ u=0$，所以
  \[
    (1\otimes q)\circ (1\otimes u)=1\otimes (q\circ u)=0.
  \]
  故 $1\otimes q\colon N\otimes_R B\to N\otimes_R(B/u(A))$ 在商上诱导出同态
  \[
    \overline q\colon (N\otimes_R B)/\Ima(1\otimes u)\to N\otimes_R(B/u(A)).
  \]

  反过来，定义双线性映射
  \[
    \beta\colon N\times (B/u(A))\to (N\otimes_R B)/\Ima(1\otimes u),
    \qquad \beta(n,\,b+u(A))=[n\otimes b].
  \]
  若 $b-b'=u(a)$，则
  \[
    n\otimes b-n\otimes b'=n\otimes (b-b')=n\otimes u(a)=(1\otimes u)(n\otimes a)
  \]
  落在 $\Ima(1\otimes u)$ 中，所以 $\beta$ 良定义；它还是平衡的，于是由张量积泛性质诱导出唯一同态
  \[
    \widetilde\beta\colon N\otimes_R(B/u(A))\to (N\otimes_R B)/\Ima(1\otimes u),
    \qquad n\otimes (b+u(A))\mapsto [n\otimes b].
  \]

  对纯张量直接计算可得
  \[
    (\overline q\circ \widetilde\beta)(n\otimes (b+u(A)))=n\otimes (b+u(A)),
  \]
  以及
  \[
    (\widetilde\beta\circ \overline q)([n\otimes b])=[n\otimes b].
  \]
  故 $\overline q$ 与 $\widetilde\beta$ 互为逆同构，即
  \[
    (N\otimes_R B)/\Ima(1\otimes u)\cong N\otimes_R(B/u(A)).
  \]

  \item \textbf{解答.} 对 $\Z$-模有自然同构 $\Z/3\Z\otimes_{\Z}\Z\cong \Z/3\Z$，故把
  \[
    0\to \Z \xrightarrow{\times 2} \Z \to \Z/2\Z \to 0
  \]
  施行函子 $\Z/3\Z\otimes_{\Z}-$ 后得到
  \[
    \Z/3\Z \xrightarrow{\times 2} \Z/3\Z \to \Z/3\Z\otimes_{\Z}\Z/2\Z \to 0.
  \]
  又由一般公式 $\Z/m\Z\otimes_{\Z}\Z/n\Z\cong \Z/\gcd(m,n)\Z$，或直接由 $2$ 与 $3$ 互素可知
  \[
    \Z/3\Z\otimes_{\Z}\Z/2\Z=0.
  \]
  因而所得序列是
  \[
    \Z/3\Z \xrightarrow{\times 2} \Z/3\Z \to 0 \to 0.
  \]
  在 $\Z/3\Z$ 上，乘以 $2$ 的映射是双射，因此尤其是满射。于是上式在最右两项处正合，这正好验证了张量积函子的右正合性。在此例中它甚至在左端也正合，因为乘以 $2$ 还是单射。

  \item \textbf{解答.} 已知对任意 $M$，函子 $\Hom_R(M,-)$ 总是左正合，所以只需证明它把满射送到满射。设
  \[
    0\to A\xrightarrow{u}B\xrightarrow{v}C\to 0
  \]
  为短正合序列。给定任意 $f\colon M\to C$，因为 $v\colon B\to C$ 是满射且 $M$ 投射，所以按投射模的定义，存在 $g\colon M\to B$ 使
  \[
    v\circ g=f.
  \]
  这说明映射
  \[
    v_*\colon \Hom_R(M,B)\to \Hom_R(M,C),
    \qquad g\mapsto v\circ g
  \]
  是满射。结合左正合性，得到
  \[
    0\to \Hom_R(M,A)\xrightarrow{u_*}\Hom_R(M,B)\xrightarrow{v_*}\Hom_R(M,C)\to 0
  \]
  正合。因此 $\Hom_R(M,-)$ 为正合函子。

  \item \textbf{解答.} 记 $\iota_S\colon S\to U(F(S))$ 为把生成元嵌入自由群的标准映射。定义
  \[
    \Phi_{S,G}\colon \Hom_{\mathbf{Grp}}(F(S),G)\to \Hom_{\mathbf{Set}}(S,U(G)),
    \qquad \varphi\mapsto U(\varphi)\circ \iota_S.
  \]
  反过来，若给定集合映射 $a\colon S\to U(G)$，由自由群的泛性质，存在唯一群同态
  \[
    \widetilde a\colon F(S)\to G
  \]
  使得 $U(\widetilde a)\circ \iota_S=a$。于是可定义
  \[
    \Psi_{S,G}(a)=\widetilde a.
  \]
  由唯一性立刻知道 $\Phi_{S,G}$ 与 $\Psi_{S,G}$ 互为逆，因此得到双射
  \[
    \Hom_{\mathbf{Grp}}(F(S),G)\cong \Hom_{\mathbf{Set}}(S,U(G)).
  \]

  该双射对 $S$ 与 $G$ 都是自然的：若 $h\colon S'\to S$ 是集合映射，或 $k\colon G\to G'$ 是群同态，则两边相应的复合都由“先在生成元上复合，再按自由群泛性质唯一延拓”刻画，所以自然交换。按照伴随函子的定义，这正说明 $F$ 是 $U$ 的左伴随，记作
  \[
    F\dashv U.
  \]

  \item \textbf{解答.} 对任意左 $R$-模 $M$，定义
  \[
    \epsilon_M\colon \Hom_R(R,M)\to M,
    \qquad f\mapsto f(1).
  \]
  其逆映射为
  \[
    \delta_M\colon M\to \Hom_R(R,M),
    \qquad m\mapsto (r\mapsto rm).
  \]
  显然 $\epsilon_M\circ \delta_M=\Id_M$ 且 $\delta_M\circ \epsilon_M=\Id$，并且对 $M\to N$ 的模同态这些同构是自然的。因此
  \[
    \Hom_R(R,-)\cong \Id_{R\text{-}\mathbf{Mod}},
  \]
  所以函子 $\Hom_R(R,-)$ 由对象 $R$ 表示，即 $R$ 就是表示对象。

  若 $P$ 是有限生成投射模，则存在有限个元 $p_i\in P$ 与对偶元 $\phi_i\in P^{\vee}=\Hom_R(P,R)$，使得对每个 $p\in P$，
  \[
    p=\sum_i \phi_i(p)p_i.
  \]
  于是对任意 $M$ 有自然同构
  \[
    \Hom_R(P,M)\cong P^{\vee}\otimes_R M.
  \]
  另一方面，投射性意味着 $P$ 是某个有限自由模 $R^n$ 的直和因子，所以 $\Hom_R(P,-)$ 是
  \[
    \Hom_R(R^n,-)\cong (-)^n
  \]
  的直和因子，也即在函子范畴中是一个可表函子的退化（retract）。这就是它“接近可表表示”的意义：它未必本身由某个对象直接表示，但它是有限个可表函子的直和因子，并且可由固定右模 $P^{\vee}$ 的张量积来描述。

  \item \textbf{解答.} 将短正合序列
  \[
    0\to \Z \xrightarrow{\times n} \Z \to \Z/n\Z \to 0
  \]
  施行函子 $\Hom_{\Z}(-,\Z/m\Z)$，并接上右导出函子，得到长正合序列的一段
  \[
    0\to \Hom_{\Z}(\Z/n\Z,\Z/m\Z)\to \Hom_{\Z}(\Z,\Z/m\Z)
    \xrightarrow{(\times n)^*} \Hom_{\Z}(\Z,\Z/m\Z)
    \to \Ext^1_{\Z}(\Z/n\Z,\Z/m\Z)\to \Ext^1_{\Z}(\Z,\Z/m\Z).
  \]
  由于 $\Z$ 是自由模，故 $\Ext^1_{\Z}(\Z,\Z/m\Z)=0$。再由 $\Hom_{\Z}(\Z,\Z/m\Z)\cong \Z/m\Z$，且预复合 $ (\times n)^*$ 在此识别下就是乘以 $n$，于是有短正合序列
  \[
    \Z/m\Z \xrightarrow{\times n} \Z/m\Z \to \Ext^1_{\Z}(\Z/n\Z,\Z/m\Z)\to 0.
  \]
  因而
  \[
    \Ext^1_{\Z}(\Z/n\Z,\Z/m\Z)
    \cong (\Z/m\Z)/n(\Z/m\Z).
  \]
  设 $d=\gcd(m,n)$。在循环群 $\Z/m\Z$ 中，乘以 $n$ 的像恰有 $m/d$ 个元素，所以其余核有 $d$ 个元素，商群也有 $d$ 个元素。由于任意有限循环群按阶唯一确定，得
  \[
    (\Z/m\Z)/n(\Z/m\Z)\cong \Z/d\Z=\Z/\gcd(m,n)\Z.
  \]
  故
  \[
    \Ext^1_{\Z}(\Z/n\Z,\Z/m\Z)\cong \Z/\gcd(m,n)\Z.
  \]
\end{enumerate}
