...
本章将以最典型的二阶线性方程为主线：先从简谐运动出发，再讨论单摆、阻尼振动和受迫振动，最后进入波动方程、干涉与驻波，并用高考题说明微积分视角如何把“记结论”变成“会推导”。
...
\begin{exercise}
设质点做简谐运动 $x=A\cos(\omega t+\varphi)$。证明速度与位移满足
\[
v^2=\omega^2\left(A^2-x^2\right).
\]
并据此说明为什么在平衡位置速度最大，在端点速度为零。
\end{exercise}

\begin{exercise}
某弹簧振子周期为 $0.50\text{ s}$，振幅为 $8\text{ cm}$。求角频率、最大速度与最大加速度。
\end{exercise}
...
\begin{exercise}
从单摆精确方程 \eqref{eq:pendulum-exact} 出发，解释为什么“小角近似”本质上是把非线性项 $\sin\theta$ 用线性项 $\theta$ 代替。非线性会给周期带来什么影响？
\end{exercise}

\begin{exercise}
设某单摆长度为 $0.64\text{ m}$。求其小角近似周期。若要使周期约为 $2.0\text{ s}$，摆长应调到多少？
\end{exercise}
...
\begin{exercise}
判断下列方程对应的阻尼类型，并写出解的基本形式：
\[
\text{(1)}\ \ddot x+4\dot x+4x=0,
\qquad
\text{(2)}\ \ddot x+10\dot x+9x=0.
\]
\end{exercise}

\begin{exercise}
某欠阻尼振子振幅满足 $A(t)=A_0 e^{-\gamma t}$。若经过 $5\text{ s}$ 振幅衰减为初值的 $1/2$，求 $\gamma$；再求经过同样时间后能量衰减为初值的几分之几。
\end{exercise}
...
\begin{tikzpicture}[scale=1.0]
  \draw[->] (0,0) -- (6.4,0) node[right] {$\Omega$};
  \draw[->] (0,0) -- (0,3.8) node[above] {$X$};
  \draw[thick, blue!70!black, smooth] plot coordinates {
    (0.35,0.4) (0.8,0.55) (1.4,0.9) (2.0,1.6) (2.5,3.0) (2.8,3.3) (3.1,2.5) (3.6,1.5) (4.2,0.95) (5.0,0.6) (5.8,0.42)
  };
  \draw[dashed] (2.8,0) -- (2.8,3.3);
  \node[below] at (2.8,0) {$\Omega_r$};
  \node[above right] at (3.4,2.9) {共振峰};
\end{tikzpicture}
\caption{阻尼受迫振动的振幅—频率曲线示意图}
\end{figure}
...
\]
当 $\Omega\neq\omega_0$ 时，求一个特解；并讨论当 $\Omega\to\omega_0$ 时该特解的振幅会发生什么变化。
\end{exercise}
...
\begin{exercise}
证明：若 $y=A\sin(kx+\omega t)$，则它也是波动方程的解，并指出它沿哪个方向传播。
\end{exercise}
...
\begin{tikzpicture}[scale=0.95]
  ...
  \draw[thick, blue!70!black, domain=0:6.1, samples=180] plot (\x,{1.2*sin(deg(2*\x))*cos(deg(0.0))});
  \foreach \p in {0.785,2.356,3.927,5.498} {
    \draw[dashed] (\p,-1.5) -- (\p,1.5);
  }
  \node[below] at (0.785,-1.5) {波节};
  \node[below] at (2.356,-1.5) {波节};
  \node[above] at (1.57,1.35) {波腹};
  \node[above] at (4.71,1.35) {波腹};
\end{tikzpicture}
\caption{驻波的瞬时形状示意图}
\end{figure}
...
y=2A\cos kx\cos\omega t
\]
求相邻两个波节之间的距离，并说明它与波长的关系。
\end{exercise}

\begin{exercise}
两列声波频率分别为 $440\text{ Hz}$ 和 $444\text{ Hz}$。求拍频，并求声音强弱变化的周期。
\end{exercise}
...
\begin{gaokao}[title={\bfseries 高考题1：弹簧振子的图像与微分方程}]
某弹簧振子经过平衡位置时速度大小为 $v_0$，最大位移为 $A$。求其角频率，并写出从最大位移处开始计时的位移函数。

\textbf{微积分视角：} 在平衡位置 $x=0$ 处，简谐运动速度达到最大值，故
\[
v_{\max}=\omega A=v_0.
\]
于是
...
\ddot x+\omega^2x=0
\]
的通解 $x=C_1\cos\omega t+C_2\sin\omega t$，立得
\[
C_1=A,
\qquad
C_2=0.
...
因此
\[
x(t)=A\cos\frac{v_0}{A}t.
\]
这个解法的关键不是“记住某个图像”，而是抓住二阶方程与初值条件。
\end{gaokao}

\begin{gaokao}[title={\bfseries 高考题2：单摆周期为什么近似不变}]
某实验中，同一单摆在小振幅下多次摆动，测得周期几乎不随振幅变化。请解释这一现象；并指出当振幅增大时会出现怎样的系统偏差。

\textbf{微积分视角：} 单摆精确方程为
\[
\ddot\theta+\frac{g}{\ell}\sin\theta=0.
\]
在小角度条件下，$\sin\theta\approx\theta$，于是方程线性化为
\[
\ddot\theta+\frac{g}{\ell}\theta=0,
\]
故周期近似为
...
T\approx 2\pi\sqrt{\frac{\ell}{g}},
\]
其中不含振幅，所以实验上会观察到“近似等时性”。

但若振幅不可忽略，精确周期满足
\[
T=2\pi\sqrt{\frac{\ell}{g}}\left(1+\frac{\theta_0^2}{16}+\cdots\right),
\]
因此振幅越大，周期越长。若仍套用小角公式，就会把理论周期估计得偏小。
\end{gaokao}

\begin{gaokao}[title={\bfseries 高考题3：波形传播与质点振动的区分}]
已知某时刻绳上的波形为正弦曲线，并知波沿 $+x$ 方向传播。如何写出波函数？又如何判断绳上某一点下一时刻是向上运动还是向下运动？

\textbf{微积分视角：} 若波沿 $+x$ 方向传播，波函数一般写作
\[
y(x,t)=A\cos(kx-\omega t+\varphi).
\]
其中负号“$-\omega t$”体现了波峰随时间向右移动。若已知 $t=0$ 时的波形为
\[
y(x,0)=A\cos(kx+\varphi),
\]
则只需把空间表达式中的 $x$ 替换为 $x-vt$ 即可得到行波解。

判断某点下一时刻的运动方向，本质上是看该点的速度
\[
\pdv{y}{t}=\omega A\sin(kx-\omega t+\varphi).
\]
在指定时刻、指定位置代入后：若 $\partial y/\partial t>0$，则质点向上运动；若 $\partial y/\partial t<0$，则质点向下运动。这说明“波向哪边传”和“介质点往哪边动”是两个不同问题，前者看相位 $kx-\omega t$ 的传播，后者看偏导数 $\partial y/\partial t$ 的符号。
\end{gaokao}
