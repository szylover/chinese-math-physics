...
当质量恒定时，
\[
\dv{\vb p}{t}=\dv{}{t}(m\vb v)=m\dv{\vb v}{t}=m\vb a,
\]
...
\begin{tikzpicture}[scale=1.0,>=Stealth]
  ...
  \draw[->] (0,0) -- (0,3.6) node[above] {$F$};
  \draw[thick,blue!70!black] (0.6,0) .. controls (1.5,2.4) and (3.0,3.0) .. (4.8,0);
  \draw[dashed] (0.6,0) node[below] {$t_1$} -- (0.6,0);
  \draw[dashed] (4.8,0) node[below] {$t_2$} -- (4.8,0);
  \fill[blue!20,opacity=0.8] (0.6,0) .. controls (1.5,2.4) and (3.0,3.0) .. (4.8,0) -- cycle;
  \node at (2.8,1.0) {$\displaystyle I=\int_{t_1}^{t_2}F\,\dd t$};
\end{tikzpicture}
\caption{变力冲量等于 $F$--$t$ 图像下的面积}
\end{figure}
...
两式相加得
\[
\dv{}{t}(\vb p_1+\vb p_2)=\vb F_1^{\text{ext}}+\vb F_2^{\text{ext}}+(\vb F_{12}+\vb F_{21}).
\]
若满足牛顿第三定律，
\[
\vb F_{12}=-\vb F_{21},
...
对全部 $i$ 求和，得到
\[
\dv{}{t}\sum_{i=1}^N\vb p_i=\sum_{i=1}^N\vb F_i^{\text{ext}}+\sum_{i=1}^N\sum_{j\ne i}\vb F_{ij}.
\]
...
等价地，
\[
\dv{}{t}(M\vb V_{\text{cm}})=\vb F_{\text{ext}}.
\]
...
\[
\vb F_{\text{ext}}=\dv{}{t}(m\vb v)-\vb u\,\dv{m}{t},
\]
...
\begin{gaokao}[title={高考题的微积分视角：子弹射入木块再压缩弹簧}]
\textbf{题意概述：} 质量为 $m$ 的子弹以速度 $v_0$ 射入静止木块，木块质量为 $M$，二者粘连后沿光滑水平面运动并压缩劲度系数为 $k$ 的轻弹簧。求弹簧最大压缩量。

\textbf{微积分视角的关键分段：}
\begin{enumerate}
  \item \textbf{碰撞极短阶段：} 取“子弹+木块”为系统，忽略外冲量，用动量守恒：
  \[
  mv_0=(M+m)v.
  \]
  得
  ...
  v=\frac{m}{M+m}v_0.
  \]
  \item \textbf{压缩弹簧阶段：} 此后已不是碰撞问题，而是机械能转化问题。设最大压缩量为 $x$，则
  \[
...
代入碰后速度得到
\[
 x=v_0\sqrt{\frac{m^2}{k(M+m)}}.
\]

\textbf{点评：} 高考常把“碰撞”和“后续运动”放在一起。真正高效的做法不是一条公式到底，而是先用冲量--动量处理短时强作用，再对后续较长过程改用能量方法。微积分语言帮助我们理解：两阶段对应两种不同的时间尺度。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：爆炸后碎片落点问题}]
\textbf{题意概述：} 某物体做平抛运动，在飞行过程中炸成两块，已知其中一块恰好竖直下落。问另一块将落在何处，或求其速度大小。

\textbf{微积分视角：}
爆炸持续时间极短，空气阻力忽略时外冲量近似只来自重力，但其冲量在极短时间内可忽略。因此爆炸瞬间两碎片系统动量守恒。

若爆炸前在某时刻总动量为
\[
\vb P=M\vb v,
\]
爆炸后其中一块动量为 $\vb p_1$，另一块动量必为
\[
\vb p_2=\vb P-\vb p_1.
\]
若题目给出“第一块竖直下落”，则它的水平动量为零，于是第二块承担全部水平动量。进一步利用质量关系即可求其水平速度；若再结合竖直方向动量守恒，也可求完整速度。

\textbf{本质说明：} 爆炸后各块轨迹很复杂，但系统质心轨迹完全不变。高考题常通过某一块的特殊运动（如竖直下落、静止、速度方向已知）来简化未知量，本质仍是分方向动量守恒与质心运动定理。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：小车与滑块的相对运动}]
\textbf{题意概述：} 光滑水平面上小车质量为 $M$，车上有质量为 $m$ 的滑块。滑块在车上受某随时间变化的水平作用力而相对滑动，求小车速度、位移或系统质心运动。

\textbf{微积分解读：}
若取“滑块+小车”为系统，水平方向外力为零，则始终有
\[
P_x=Mv_{\text{车}}+mv_{\text{块}}=\text{常量}.
\]
若系统初始静止，则
\[
Mv_{\text{车}}+mv_{\text{块}}=0.
\]
即使滑块所受推力随时间变化，只要它来自系统内部，内力冲量在系统层面总会抵消。进一步地，系统质心满足
\[
X_{\text{cm}}=\frac{MX_{\text{车}}+mx_{\text{块}}}{M+m}=\text{常量或匀速直线运动}.
\]

\textbf{点评：} 这类题目表面上像牛顿第二定律题，但一旦涉及整体位移、相对运动和时间变化内力，质心与动量往往比逐个受力分析更快、更稳。
\end{gaokao}
...
\begin{exercise}
一质量为 $0.50\,\mathrm{kg}$ 的物体以速度
\[
\vb v=(6\vb e_x-8\vb e_y)\,\mathrm{m/s}
\]
运动。求其动量矢量的分量、大小与方向。
\end{exercise}

\begin{exercise}
一维变力满足 $F(t)=kt^2$，其中 $0\le t\le T$。若物体初速度为零、质量为 $m$，求 $t=T$ 时的速度。
\end{exercise}

\begin{exercise}
某物体所受力的 $F$--$t$ 图像是底为 $2\,\mathrm{s}$、高为 $10\,\mathrm{N}$ 的三角形。求该过程的冲量，并说明若初动量为 $4\,\mathrm{kg\cdot m/s}$，末动量是多少。
\end{exercise}

\begin{exercise}
两小球在光滑水平面上相向运动，质量分别为 $m$ 和 $2m$，速度分别为 $4v$ 和 $-v$。若碰后粘在一起，求共同速度。
\end{exercise}

\begin{exercise}
设质量分别为 $m_1,m_2$ 的两物体做一维弹性碰撞，碰前 $m_2$ 静止。利用动量守恒与机械能守恒，推导碰后速度公式
\[
...
\begin{exercise}
证明：一维完全非弹性碰撞中损失的机械能可写成
\[
\Delta E=\frac12 \frac{m_1m_2}{m_1+m_2}(u_1-u_2)^2.
\]
并据此说明只有当两物体碰前速度相同，机械能损失才为零。
\end{exercise}

\begin{exercise}
三个质点质量分别为 $m,2m,3m$，在 $x$ 轴上的位置分别为 $0,L,4L$。求系统质心坐标。
\end{exercise}

\begin{exercise}
一火箭在深空中由静止开始喷气，喷气相对速度恒为 $u$。若其质量随时间满足
\[
m(t)=m_0-\lambda t,
\qquad 0\le t<\frac{m_0-m_f}{\lambda},
\]
求速度 $v(t)$，并写出燃料耗尽时的末速度。
\end{exercise}

\begin{exercise}
一质量为 $M$ 的小车静止于光滑水平面上，车上有一质量为 $m$ 的人。人以相对车速度 $u$ 向左跳出。求小车获得的速度，并用质心不动的观点重新解释你的结果。
\end{exercise}
