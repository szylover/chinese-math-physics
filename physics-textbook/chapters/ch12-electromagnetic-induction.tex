...
下面选取三道典型高考风格题，展示如何把“公式套用”提升为“函数、导数与积分”的统一理解。

\begin{gaokao}[title={\bfseries 高考题 1：导体棒切割磁力线}]
已知两平行导轨间距为 $\ell$，导轨电阻不计，左端接电阻 $R$。一根质量为 $m$、电阻不计的金属棒在竖直向下的匀强磁场 $B$ 中沿导轨向右滑动。若某时刻棒的速度为 $v$，求：
\begin{enumerate}
  \item 回路中的感应电流；
  \item 棒所受安培力；
  \item 若仅受安培力阻碍，写出速度满足的微分方程。
\end{enumerate}
...
安培力大小
\[
F=B I\ell=\frac{B^2\ell^2}{R}v,
\]
方向与速度反向。于是由牛顿第二定律
\[
m\dv{v}{t}=-\frac{B^2\ell^2}{R}v.
\]
这已经是一个可分离变量方程，解为
\[
 v(t)=v_0\exp\left(-\frac{B^2\ell^2}{mR}t\right).
\]
可见电磁阻尼使速度指数衰减，而不是匀减速。这正是“受力与速度成正比”所对应的微分方程特征。
\end{gaokao}

\begin{gaokao}[title={\bfseries 高考题 2：线圈中磁通量随时间变化}]
一匝数为 $N$、面积为 $A$ 的线圈固定不动，所在区域磁场垂直线圈平面且满足
\[
B(t)=B_0+kt^2 \qquad (k>0).
\]
设线圈总电阻为 $R$。求：
\begin{enumerate}
  \item 任意时刻的感应电动势；
  \item 任意时刻的感应电流；
  \item 从 $0$ 到 $T$ 时间内通过线圈某截面的电荷量。
\end{enumerate}

\textbf{微积分解：}
磁通量为
\[
\Phi_B(t)=B(t)A=(B_0+kt^2)A.
\]
于是
\[
\mathcal{E}(t)=-N\dv{\Phi_B}{t}=-N\cdot 2ktA.
\]
故感应电流为
\[
I(t)=\frac{\mathcal{E}(t)}{R}=-\frac{2NkAt}{R}.
\]
若求通过某截面的电荷量，只需积分电流：
\[
Q=\int_0^T I(t)\,\dd t
=-\frac{2NkA}{R}\int_0^T t\,\dd t
=-\frac{NkAT^2}{R}.
\]
故电荷量大小为
\[
|Q|=\frac{NkAT^2}{R}.
\]
这道题最值得注意的地方是：\textbf{电荷量不是用某个瞬时公式代入，而是对电流函数做定积分。}
\end{gaokao}

\begin{gaokao}[title={\bfseries 高考题 3：线框出入磁场区域}]
边长为 $a$、总电阻为 $R$ 的正方形线框，以恒定速度 $v$ 水平穿过宽度大于 $a$ 的匀强磁场区域，磁场方向垂直纸面向里，磁感应强度为 $B$。求线框刚进入磁场过程中的感应电动势与感应电流，并说明为何在线框完全进入后电流变为零。

\textbf{微积分解：}
设线框进入深度为 $x(t)=vt$，在刚进入阶段 $0\le x\le a$。此时处于磁场中的有效面积为
\[
A_{\text{in}}(t)=a\,x(t)=avt.
\]
故磁通量
\[
\Phi_B(t)=B A_{\text{in}}(t)=Bavt.
\]
感应电动势
\[
\mathcal{E}=-\dv{\Phi_B}{t}=-Bav.
\]
其大小恒为 $Bav$，故电流大小为
\[
I=\frac{Bav}{R}.
\]
当线框完全进入磁场后，有效面积恒为 $a^2$，于是
\[
\Phi_B=Ba^2=\text{常量},
\qquad
\dv{\Phi_B}{t}=0.
\]
所以虽然线框仍在磁场中运动，但磁通量不再变化，感应电流便消失。由此可见，“是否在磁场中”不是关键，\textbf{关键在于穿过回路的磁通量是否随时间变化。}
\end{gaokao}
...
\begin{exercise}
设一平面线圈面积为 $A$，其法线与匀强磁场夹角满足 $\theta(t)=\alpha t^2$。写出磁通量 $\Phi_B(t)$ 与感应电动势 $\mathcal{E}(t)$ 的表达式。
\end{exercise}

\begin{exercise}
半径为 $R$ 的圆线圈处在磁场 $B(t)=B_0 e^{-\lambda t}$ 中，磁场始终垂直线圈平面。求线圈中感应电动势随时间的函数关系，并判断其方向如何变化。
\end{exercise}

\begin{exercise}
一长度为 $\ell$ 的金属棒以速度 $v(x)=v_0+kx$ 在匀强磁场中沿导轨滑动。若回路总电阻为 $R$，求感应电流关于位置 $x$ 的函数关系，并写出单位时间内产生的焦耳热功率。
\end{exercise}

\begin{exercise}
证明：对固定回路，若穿过回路的磁通量保持常量，则尽管磁场可能在空间中重新分布，回路中的总感应电动势仍可为零。请结合法拉第定律说明原因。
\end{exercise}

\begin{exercise}
一长直螺线管长度为 $l$，截面积为 $A$，总匝数为 $N$。在空气中近似认为内部磁场均匀。试推导其电感公式
\[
L=\mu_0 \frac{N^2A}{l}.
\]
并说明该结果与 $L=\mu_0 n^2Al$ 的一致性。
\end{exercise}

\begin{exercise}
某理想变压器原线圈匝数为 $500$，副线圈匝数为 $100$。若原线圈输入交流有效值电压为 $220\,\mathrm{V}$，求副线圈输出电压；若副线圈电流为 $2\,\mathrm{A}$，求原线圈电流。
\end{exercise}

\begin{exercise}
一 RL 串联电路中，$L=0.20\,\mathrm{H}$，$R=10\,\Omega$，电源电动势为 $12\,\mathrm{V}$。求时间常数 $\tau$、稳定电流，以及接通后 $0.010\,\mathrm{s}$ 时的电流大小。
\end{exercise}

\begin{exercise}
证明感应电场不是保守场，即说明在变化磁场存在时沿闭合回路有
\[
\oint \vb{E}\cdot \dd \vb{l}\neq 0.
\]
并比较它与静电场 $\oint \vb{E}\cdot \dd \vb{l}=0$ 的根本差别。
\end{exercise}

\begin{exercise}
一闭合金属环从无磁场区域进入磁场区域，又完全离开磁场区域。请画出磁通量 $\Phi_B(t)$、感应电动势 $\mathcal{E}(t)$ 与感应电流 $I(t)$ 的大致图像，并说明三个图像之间的导数关系。
\end{exercise}

\begin{exercise}
设某区域磁场能量密度满足 $u_B=\dfrac{B^2}{2\mu_0}$。若该区域体积为 $V$，且磁场近似均匀为 $B$，证明总磁场能量为 $U_B=u_BV$。再将其与长螺线管的 $\dfrac{1}{2}LI^2$ 比较，说明两种表达式为何一致。
\end{exercise}
