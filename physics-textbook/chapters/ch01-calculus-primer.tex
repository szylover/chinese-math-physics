...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale=0.95]
    ...
    \draw[thick, blue, domain=0.5:5.0, samples=100] plot (\x, {0.35*\x + 0.25*sin(120*\x) + 0.6});
    \fill[blue!15, domain=1:4, samples=80] (1,0) -- plot (\x, {0.35*\x + 0.25*sin(120*\x) + 0.6}) -- (4,0) -- cycle;
    \draw[dashed] (1,0) -- (1,1.1);
    \draw[dashed] (4,0) -- (4,2.0);
    \node at (1,-0.25) {$a$};
    \node at (4,-0.25) {$b$};
  \end{tikzpicture}
  \caption{定积分的几何意义：曲线下方的有向面积}
\end{figure}
...
\begin{exercise}
已知位置函数 $x(t)=t^3-6t^2+9t$。
\begin{enumerate}
  \item 求速度 $v(t)$ 与加速度 $a(t)$；
  \item 求 $t=1$ 时与 $t=3$ 时的速度；
  \item 判断物体在哪些时刻瞬时静止。
\end{enumerate}
\end{exercise}
...
设速度函数为 $v(t)=4t-1$。
\begin{enumerate}
  \item 求 $t=0$ 到 $t=3$ 之间的位移；
  \item 求 $t=0$ 到 $t=3$ 之间的路程；
  \item 说明位移与路程为何不同。
\end{enumerate}
\end{exercise}
...
求下列不定积分：
\begin{enumerate}
  \item $\int (2x^3-5x+4)\,\dd x$；
  \item $\int (\cos x + \mathrm{e}^x)\,\dd x$；
  \item $\int \dfrac{1}{x}\,\dd x$；
  \item $\int 6x\sin(3x^2)\,\dd x$.
\end{enumerate}
\end{exercise}
...
已知
\[
  \vb{A}=(1,2,3), \qquad \vb{B}=(2,-1,4).
\]
求：
\begin{enumerate}
  \item $\vb{A}+\vb{B}$ 与 $\vb{A}-\vb{B}$；
  \item $\vb{A}\cdot\vb{B}$；
  \item $\vb{A}\times\vb{B}$；
  \item 两个矢量是否互相垂直。
\end{enumerate}
...
\begin{exercise}
一质点在平面内的位移为 $\vb{r}(t)=(3t^2,\ 2t)$。
\begin{enumerate}
  \item 求速度矢量 $\vb{v}(t)$；
  \item 求加速度矢量 $\vb{a}(t)$；
  \item 求 $t=1$ 时速度的大小；
  \item 说明“速度大小”和“速度矢量”有何区别。
\end{enumerate}
\end{exercise}
