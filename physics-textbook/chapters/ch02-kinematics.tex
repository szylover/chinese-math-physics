...
\begin{tikzpicture}[scale=0.9, >=Latex]
  \draw[->] (-0.2,0) -- (7.4,0) node[below] {$x$};
  \draw[->] (0,-0.2) -- (0,4.8) node[left] {$y$};
  \draw[thick, blue, domain=0:6.3, smooth] plot(\x,{0.9*\x-0.12*\x*\x});
  \draw[->, thick, red] (0,0) -- (1.8,1.6) node[midway, above] {$\vb{v}_0$};
  \draw (1.0,0) arc (0:41:1.0);
  \node at (1.35,0.35) {$\theta$};
  \node[blue] at (5.5,1.9) {轨迹};
\end{tikzpicture}
\caption{无空气阻力时的抛体轨迹}
\end{figure}
...
下面选取三类经典高考运动学题。每一道题先用微积分重述，再与高中常规解法对照。读者会发现：高中方法并没有错，只是它往往把微积分已经做完后的结果直接拿来使用。

\begin{gaokao}[title={高考题的微积分视角：题一 竖直上抛的最高点}]
\textbf{题目：} 一小球从地面竖直上抛，初速度为 $v_0$，忽略空气阻力，重力加速度为 $g$。求最高点高度与回到出发点所需时间。
...
再对速度积分：
\[
 y(t)=\int_0^t (v_0-g\tau)\,\dd \tau=v_0 t-\frac12 gt^2.
\]
于是最高点高度为
\[
 H=y(t_{\max})=\frac{v_0^2}{2g}.
\]
回到原点满足 $y(T)=0$，即
\[
 v_0T-\frac12 gT^2=0,
...
\textbf{与高中方法对比：}
高中通常直接套用 $v^2-v_0^2=2a\Delta y$ 与对称性。微积分方法则把``最高点速度为零''解释为导数为零、把高度解释为速度积分。这样一来，若以后加速度不是常量，思路仍可保留。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：题二 汽车刹车距离}]
\textbf{题目：} 汽车以速度 $v_0$ 匀减速刹车，已知刹车加速度大小为 $a$。求刹车距离。

\textbf{微积分解：}
设速度方向为正，则加速度为 $-a$。由于题目只问距离，不问时间，最适合使用链式法则：
\[
 \dv{v}{t}=-a,
 \qquad
...
\[
 v\dv{v}{x}=-a.
\]
从 $(x_0,v_0)$ 积分到停车点 $(x,0)$：
\[
 \int_{v_0}^{0} v\,\dd v=\int_{x_0}^{x} (-a)\,\dd x.
\]
得
\[
 -\frac12 v_0^2=-a(x-x_0),
\]
于是
\[
 x-x_0=\frac{v_0^2}{2a}.
...
\textbf{与高中方法对比：}
高中直接记忆结论 $v^2-v_0^2=2a\Delta x$。微积分的好处是：你知道这条式子从哪里来，也知道当 $a$ 改成 $a(x)$ 时该如何替换，而不是被公式卡住。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：题三 斜抛最大射程}]
\textbf{题目：} 在同一水平面上以固定初速度 $v_0$ 斜向上抛出小球，忽略空气阻力。问仰角 $\theta$ 取何值时射程最大？
...
要使射程最大，只需对角度求极值。因为
\[
 \dv{R}{\theta}=\frac{2v_0^2}{g}\cos 2\theta,
\]
令其为零得
\[
 \cos 2\theta=0 \quad \Rightarrow \quad 2\theta=\frac{\pi}{2},
\]
故
\[
...
再由
\[
 \dv[2]{R}{\theta}=-\frac{4v_0^2}{g}\sin 2\theta
\]
知在 $45^\circ$ 处二阶导数为负，因此确为最大值点。

\textbf{与高中方法对比：}
高中常用 $\sin 2\theta\le 1$ 立即得出结论。这当然更快；但微积分方法强调``最值问题就是导数为零再判别''这一普适原则。以后遇到有阻力、斜坡着地、发射高度不同时，三角恒等式未必够用，而求导思路仍然成立。
\end{gaokao}
...
\[
 v(t)=3t^2-6t+2.
\]
已知 $x(0)=1$。求 $x(t)$，并求从 $t=0$ 到 $t=3$ 的位移与路程。
\end{exercise}

\begin{exercise}
某物体做匀变速直线运动，已知 $x_0=0$，$v_0=5\,\text{m/s}$，$a=-2\,\text{m/s}^2$。求：
\begin{enumerate}
  \item 任意时刻的速度与位置；
  \item 速度第一次变为零的时刻；
  \item 该时刻前的最大位移。
...
\[
 a(t)=4-2t.
\]
已知 $v(0)=1$，$x(0)=0$。用积分求 $v(t)$ 与 $x(t)$，并判断何时加速度变号。
\end{exercise}

\begin{exercise}
已知一质点从静止出发，其加速度与位置满足
\[
 a(x)=kx \qquad (k>0).
\]
利用
...
\begin{exercise}
一小球以初速度 $v_0$ 从地面竖直上抛，忽略空气阻力。请用微积分方法推导最高点高度，并说明为什么最高点对应于位置函数的极大值点。
\end{exercise}

\begin{exercise}
一物体以初速度 $v_0$、仰角 $\theta$ 做斜抛运动，忽略空气阻力。推导其轨迹方程
\[
 y=x\tan\theta-\frac{g}{2v_0^2\cos^2\theta}x^2,
\]
并由此解释轨迹为何是抛物线。
...
\dv{v_y}{t}=-g-kv_y.
\]
若初速度水平，即 $v_x(0)=v_0$、$v_y(0)=0$，求 $v_x(t)$、$v_y(t)$ 与 $y(t)$。
\end{exercise}

\begin{exercise}
设质点做半径为 $R$ 的圆周运动，角位置为
\[
 \theta(t)=\omega_0 t+\frac12 \alpha t^2.
\]
求：
\begin{enumerate}
  \item 角速度 $\omega(t)$；
...
\begin{exercise}
阅读本章后，请用不超过 200 字的话说明：为什么说``高中运动学公式其实都是微积分结论的特殊情形''？并任选一个公式写出它的微积分推导线索。
\end{exercise}
