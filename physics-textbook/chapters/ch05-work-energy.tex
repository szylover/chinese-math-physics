...
\begin{tikzpicture}[scale=0.95]
  ...
  \draw[->] (0,-0.2) -- (0,3.2) node[above] {$F$};
  \draw[thick,blue] (0,0) -- (4,2.8) node[above right] {$F=kx$};
  \draw[dashed] (4,0) node[below] {$a$} -- (4,2.8);
  \fill[blue!18] (0,0) -- (4,0) -- (4,2.8) -- cycle;
  \node at (2.6,0.8) {$W=\displaystyle\int_0^a F\,\dd x$};
\end{tikzpicture}
\end{center}
\end{example}
...
左边就是元功 $\dd W$。右边利用恒等式
\[
\dv{}{t}(v^2)=\dv{}{t}(\vb{v}\cdot \vb{v})=2\vb{v}\cdot\dv{\vb{v}}{t},
\]
得
\[
...
\section{高考题微积分视角}\label{sec:gaokao-work-energy}

下面选取三类经典题：变力做功、弹簧问题、传送带问题。我们的目标不是重复常规解法，而是展示微积分语言如何把图像、过程与公式统一起来。

\begin{gaokao}[title={高考题的微积分视角：变力做功}]
某水平轨道上，物体从 $x=0$ 运动到 $x=L$，受水平拉力
\[
F(x)=F_0\qty(1-\frac{x}{L})
\qquad (0\le x\le L)
\]
作用，忽略摩擦。若初速度为零，求末速度。
...
拉力是随位移线性减小的变力，故先算功：
\[
W=\int_0^L F_0\qty(1-\frac{x}{L})\dd x
=F_0\qty[x-\frac{x^2}{2L}]_0^L
=\frac{1}{2}F_0L.
\]
再由动能定理
\[
...
\textbf{点评：}
许多试题会提示作 $F-x$ 图像。其本质并非“套面积法”，而是把线积分
\[
\int_0^L F(x)\,\dd x
\]
几何化。图像面积法只是积分思想的一个可视化版本。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：弹簧与最大压缩量}]
质量为 $m$ 的小物块以初速度 $v_0$ 沿光滑水平面滑向轻弹簧，弹簧劲度系数为 $k$。求弹簧最大压缩量 $x_{\max}$。

\textbf{微积分解：}
从接触弹簧到压缩最大处，系统只有弹力做功，机械能守恒：
\[
...
若坚持从功的定义出发，也可写为：压缩到 $x$ 时弹簧力为 $F=-kx$，于是弹簧做功
\[
W_s=\int_0^{x_{\max}} (-kx)\,\dd x=-\frac{1}{2}kx_{\max}^2.
\]
再由动能定理
\[
-\frac{1}{2}kx_{\max}^2=0-\frac{1}{2}mv_0^2,
\]
得到同样结果。

\textbf{点评：}
“弹簧势能 $\frac{1}{2}kx^2$”并不是凭空记忆出来的，而是对胡克力积分后的自然结果。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：传送带与摩擦做功}]
水平传送带以恒速 $v$ 向右运动。质量为 $m$ 的物块以初速度零轻放到传送带上，物块与带之间动摩擦因数为 $\mu$，最终与传送带共速。求摩擦力对物块所做的功，并说明机械能变化。
...
f=\mu mg,
\]
方向向右，因此物块加速度为 $a=\mu g$。由运动学可得达到速度 $v$ 的位移
\[
v^2=2as \quad\Longrightarrow\quad s=\frac{v^2}{2\mu g}.
\]
于是摩擦力对物块做功
\[
W_f=fs=\mu mg\cdot \frac{v^2}{2\mu g}=\frac{1}{2}mv^2.
\]
这恰好等于物块动能的增加：
\[
W_f=\Delta E_k.
...
\textbf{进一步分析：}
对“物块”系统而言，摩擦力是外力，且做正功，所以物块机械能增加；
但对“物块+传送带+电动机”整体而言，摩擦是内力，系统总机械能并不守恒，额外能量来自电动机提供的非机械能输入，同时伴随热耗散。

\textbf{点评：}
传送带题的关键，不在于“摩擦一定阻碍运动”，而在于分清：摩擦力到底在给哪个系统输送能量。
\end{gaokao}
...
\begin{exercise}
质点沿 $x$ 轴运动，受力 $F(x)=3x^2+2$（单位制已略）作用，从 $x=1$ 运动到 $x=3$。求该力做功。
\end{exercise}

\begin{exercise}
一恒力 $\vb{F}$ 作用在物体上，物体发生位移 $\vb{s}$。若将位移大小扩大为原来的 $2$ 倍，同时使力与位移夹角由 $60^\circ$ 变为 $120^\circ$，求功如何变化。
\end{exercise}

\begin{exercise}
质量为 $m$ 的小球在竖直方向上升过程中只受重力作用，初速度为 $v_0$。用动能定理求其上升最大高度，并与运动学公式对照。
\end{exercise}

\begin{exercise}
一质量为 $m$ 的物块在水平粗糙面上滑行，初速度为 $v_0$，动摩擦因数为 $\mu$。试用动能定理求停止前滑行距离。
\end{exercise}

\begin{exercise}
已知一维势能函数
\[
U(x)=U_0\qty(\frac{x^4}{a^4}-2\frac{x^2}{a^2}),
\]
求对应的力 $F(x)$，并找出全部平衡点与其稳定性。
\end{exercise}

\begin{exercise}
弹簧劲度系数为 $k$，一端固定，另一端与质量为 $m$ 的小块相连。将小块从平衡位置拉至 $x=A$ 后由静止释放。写出系统机械能表达式，并指出小块经过平衡位置时速度大小。
\end{exercise}

\begin{exercise}
汽车发动机额定功率为 $P$，在水平路面上匀速行驶时所受阻力恒为 $f$。求该汽车能达到的最大速度，并说明为什么速度越高加速越困难。
\end{exercise}

\begin{exercise}
某粒子的势能图在 $x=x_0$ 处有局部最小值。请用文字和数学式说明为什么这一点对应稳定平衡，并解释“小振动”时恢复力方向的来源。
\end{exercise}

\begin{exercise}
水平传送带以速度 $v$ 匀速运行，物块以同向速度 $2v$ 轻放其上。设动摩擦因数为 $\mu$，求物块最终与传送带共速前，摩擦力对物块做的功，并判断该功的正负。
\end{exercise}

\begin{exercise}
设质点在一维保守力场中运动，总机械能为 $E$，势能函数为 $U(x)$。试证明转折点满足 $U(x)=E$，并说明为什么粒子不能进入 $U(x)>E$ 的区域。
\end{exercise}
