...
下面选取三类常见高考题，展示如何把“定义、求导、积分”真正用起来。

\begin{gaokao}[title={题型一：图像给出电场，求电势差与运动趋势}]
已知某直线上电场沿 $x$ 方向，场强随位置变化为
\[
E_x=E_0-kx\qquad (0\le x\le a).
\]
设 $x=0$ 处电势为 $V_0$。

\textbf{问题：}
\begin{enumerate}
  \item 求任意位置 $x$ 处的电势 $V(x)$；
  \item 若 $k>0$，判断正电荷在该区域内更容易向哪一侧自发运动。
\end{enumerate}
...
从 $0$ 积分到 $x$：
\[
V(x)-V_0=-\int_0^x (E_0-k\xi)\,\dd \xi=-E_0x+\frac{1}{2}kx^2.
\]
故
\[
V(x)=V_0-E_0x+\frac{1}{2}kx^2.
\]
若正电荷自由运动，它总沿电势降低方向运动；或者等价地说沿电场方向运动。于是只需看 $E_x=E_0-kx$ 的符号即可：在 $x<\dfrac{E_0}{k}$ 的区域内，它倾向于向 $x$ 增大的方向运动。

\textbf{点评：} 高考中经常把 $E-x$ 图像、$V-x$ 图像、粒子运动联系起来。核心不是记图像，而是记住
\[
V_B-V_A=-\int_A^B E_x\,\dd x.
\]
\end{gaokao}

\begin{gaokao}[title={题型二：带电粒子经电势差加速}]
一带电粒子质量为 $m$、电荷量为 $q$，从静止开始经过电势差 $U$ 的加速后进入真空。忽略重力，求粒子速度大小。
...
\textbf{点评：} 这类题在表面上是动力学题，实质上是电势能问题。若电场分布复杂，只要能先算出电势差，运动学部分仍可直接求解。
\end{gaokao}

\begin{gaokao}[title={题型三：平行板电容器的动态分析}]
某平行板电容器接在恒压电源上，初始电容为 $C$、电压为 $U$。现将极板间距减小为原来的一半，忽略边缘效应。求电容、电荷、场强和储能如何变化。
...
\textbf{点评：} 电容器综合题的关键在于先判断约束条件：接电源意味着 $U$ 不变；断电意味着 $Q$ 不变。随后把其余量都看成 $d$ 的函数，再逐个推出即可。
\end{gaokao}
...
\begin{exercise}
由电势差定义证明：在静电场中，沿任意闭合回路搬运电荷，电场力做功为零。并解释这一定性结论为什么允许我们引入电势函数。
\end{exercise}

\begin{exercise}
一匀强电场大小为 $E$，方向沿 $x$ 轴正方向。若 $A(0)$、$B(l)$、$C(2l)$ 三点共线，求 $V_B-V_A$、$V_C-V_A$ 以及 $V_C-V_B$，并说明电势差与路径是否有关。
\end{exercise}
...
V(x,y)=ax^2-by^2,
\]
其中 $a,b>0$。求平面上任意点的电场矢量，并找出电场为零的点。
\end{exercise}

\begin{exercise}
点电荷 $+q$ 放在原点。求半径分别为 $R$ 与 $2R$ 的两球面之间的电势差，并据此说明点电荷电势为何随距离按 $1/r$ 衰减。
\end{exercise}

\begin{exercise}
半径为 $R$ 的均匀带电圆环总电荷为 $Q$。请直接由
\[
V(x)=\frac{1}{4\pi\varepsilon_0}\frac{Q}{\sqrt{x^2+R^2}}
\]
出发，求轴线上电场 $E_x$，并讨论 $x\gg R$ 时的近似形式。
\end{exercise}

\begin{exercise}
一平行板电容器极板面积为 $S$，板间距为 $d$。若先将其充电到电压 $U$ 后断开电源，再把板间距增大到 $2d$。求电容、电压、场强和储能如何变化。
\end{exercise}

\begin{exercise}
证明真空平行板电容器的储能也可由能量密度积分
\[
W=\iiint \frac{1}{2}\varepsilon_0E^2\,\dd\tau
\]
得到，并与 $W=\dfrac{1}{2}CU^2$ 对照验证。
\end{exercise}

\begin{exercise}
某电容器接在恒压电源上，向其中插入相对介电常数为 $\kappa$ 的电介质。比较插入前后 $C,Q,E,W$ 的变化；再讨论若电源断开，结论如何改变。
\end{exercise}

\begin{exercise}
一个质量为 $m$、带电量为 $q$ 的粒子从电势为 $V_A$ 的位置静止释放，运动到电势为 $V_B$ 的位置。忽略非电场力，求其速度，并分别讨论 $q>0$ 与 $q<0$ 时粒子运动方向与电势高低的关系。
\end{exercise}
