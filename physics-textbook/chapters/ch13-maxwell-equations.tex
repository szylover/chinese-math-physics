...
\[
I_d\coloneqq \dv{}{t}\int_{\Sigma} \vb{D}\cdot \dd \vb{A}.
\]
若介质为真空，$\vb{D}=\varepsilon_0\vb{E}$，则
\[
I_d=\varepsilon_0\dv{}{t}\int_{\Sigma} \vb{E}\cdot \dd \vb{A}.
\]
...
= I_{\text{free}}+\dv{}{t}\int_{\Sigma}\vb{D}\cdot \dd\vb{A}.
\]
在真空中，由 $\vb{B}=\mu_0\vb{H}$ 与 $\vb{D}=\varepsilon_0\vb{E}$ 得
\[
\oint_{\partial \Sigma} \vb{B}\cdot \dd \vb{l}
=\mu_0 I_{\text{cond}}+\mu_0\varepsilon_0\dv{}{t}\int_{\Sigma}\vb{E}\cdot \dd\vb{A}.
\]
\end{law}
...
因此位移电流为
\[
I_d=\varepsilon_0\dv{}{t}(EA)=\varepsilon_0 A \dv{E}{t}
=\dv{Q}{t}=I.
\]
...
\oint_{\partial \Sigma} \vb{E}\cdot \dd \vb{l} &= -\dv{}{t}\int_{\Sigma}\vb{B}\cdot \dd \vb{A}, \\
\oint_{\partial \Sigma} \vb{H}\cdot \dd \vb{l} &= I_{\text{free, enc}}+\dv{}{t}\int_{\Sigma}\vb{D}\cdot \dd \vb{A}.
\end{align*}
\end{law}
...
\oint_{\partial V} \vb{B}\cdot \dd \vb{A} &= 0, \\
\oint_{\partial \Sigma} \vb{E}\cdot \dd \vb{l} &= -\dv{}{t}\int_{\Sigma}\vb{B}\cdot \dd \vb{A}, \\
\oint_{\partial \Sigma} \vb{B}\cdot \dd \vb{l} &= \mu_0 I_{\text{enc}}+\mu_0\varepsilon_0\dv{}{t}\int_{\Sigma}\vb{E}\cdot \dd \vb{A}.
\end{align*}
...
\[
\dv{}{t}\int_V u\,\dd V+\oint_{\partial V}\vb{S}\cdot\dd\vb{A}+\int_V \vb{J}\cdot\vb{E}\,\dd V=0.
\]
...
\div\vb{J}+\pdv{\rho}{t}=0
\]
和高斯定律 $\div\vb{D}=\rho$ 出发，说明为什么安培定律必须包含位移电流项 $\pdv{\vb{D}}{t}$。
\end{exercise}

\begin{exercise}
设平行板电容器极板面积为 $A$，充电时板间电场按 $E(t)=\alpha t$ 增长。求板间位移电流 $I_d$，并说明它与导线中的传导电流关系如何。
\end{exercise}

\begin{exercise}
写出真空中麦克斯韦方程组的积分形式与微分形式，并分别说明每一个方程的物理意义。
\end{exercise}
...
\begin{exercise}
一列真空平面电磁波的电场振幅为 $E_0=300\ \text{V/m}$。求其磁场振幅 $B_0$、平均能量密度 $\avg{u}$ 与平均能流密度 $\avg{S}$。
\end{exercise}

\begin{exercise}
证明真空平面电磁波中电场能量密度与磁场能量密度恒相等，即 $u_E=u_B$。
\end{exercise}

\begin{exercise}
某微波炉工作频率约为 $2.45\ \text{GHz}$。估算其对应电磁波在真空中的波长，并说明它属于电磁谱的哪一波段。
\end{exercise}

\begin{exercise}
查阅并思考：为什么紫外线可用于消毒，而 X 射线可用于医学成像？请结合它们在电磁谱中的频率、波长与物质相互作用方式作简要说明。
\end{exercise}
