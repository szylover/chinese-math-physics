...
\begin{gaokao}[title={高考题的微积分视角：同步卫星与轨道半径}]
\textbf{题目：} 某同步卫星绕地球做圆周运动，周期与地球自转周期相同。试用微积分与牛顿力学说明其轨道半径满足什么关系，并解释为何同步卫星不可能位于低轨道。

\textbf{解析：} 圆轨道满足
\[
\frac{mv^2}{r}=\frac{GM_\oplus m}{r^2},\qquad v=r\omega=r\frac{2\pi}{T}.
\]
代入得
\[
\frac{m r^2\omega^2}{r}=\frac{GM_\oplus m}{r^2}
\quad\Rightarrow\quad
r^3=\frac{GM_\oplus}{\omega^2}=GM_\oplus\left(\frac{T}{2\pi}\right)^2.
\]
即
\[
r=\left[GM_\oplus\left(\frac{T}{2\pi}\right)^2\right]^{1/3}.
\]
由于 $T$ 已被“同步”条件固定为一天，所以 $r$ 也被唯一确定，不能任意取小。若轨道太低，则由
\[
T=2\pi\sqrt{\frac{r^3}{GM_\oplus}}
\]
可知周期必远小于一天，因此不可能保持相对地面静止。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：双星系统中的角速度}]
\textbf{题目：} 两颗恒星质量分别为 $m_1,m_2$，相距恒定为 $L$，绕共同质心做匀速圆周运动。求系统角速度 $\omega$。

\textbf{解析：} 设两星到质心距离分别为 $r_1,r_2$，则
\[
...
\]
对质量 $m_1$ 的恒星，由万有引力提供向心力：
\[
m_1\omega^2 r_1=G\frac{m_1m_2}{L^2}.
\]
代入 $r_1$ 并约去 $m_1$：
\[
\omega^2\frac{m_2}{m_1+m_2}L=G\frac{m_2}{L^2}.
\]
故
\[
\omega^2=G\frac{m_1+m_2}{L^3},
\qquad
\omega=\sqrt{G\frac{m_1+m_2}{L^3}}.
\]
这与单星绕固定中心的形式非常相似，只是中心质量换成了总质量。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：近地卫星变轨中的能量判断}]
\textbf{题目：} 一颗卫星原来在半径为 $r_1$ 的圆轨道上运行，经短时点火后进入半径更大的圆轨道 $r_2$。判断其速度、动能、势能、总机械能如何变化，并说明依据。

\textbf{解析：} 对圆轨道有
\[
v=\sqrt{\frac{GM}{r}},\qquad K=\frac{GMm}{2r},\qquad U=-\frac{GMm}{r},\qquad E=-\frac{GMm}{2r}.
\]
因此当 $r$ 从 $r_1$ 增大到 $r_2$ 时：
\begin{itemize}
  \item 轨道速度 $v$ 减小；
  \item 动能 $K$ 减小；
  \item 势能 $U$ 增大（数值变得不那么负）；
  \item 总机械能 $E$ 增大。
\end{itemize}
很多同学会疑惑：既然点火加速，为什么最终轨道速度反而变小？原因在于点火增加的是总机械能，使卫星进入更高轨道；而在更高轨道上，维持圆周运动所需的平衡速度本来就更小。
\end{gaokao}
...
\begin{exercise}
证明：若把地球看成半径为 $R$、质量为 $M$ 的球对称天体，则距地心 $r$ 处的圆轨道速度满足 $v\propto r^{-1/2}$。
\end{exercise}
...
U(r)=-\frac{GMm}{r}
\]
出发，利用 $F_r=-\dv{U}{r}$ 再次推出万有引力公式。
\end{exercise}

\begin{exercise}
一颗卫星在半径为 $r$ 的圆轨道上运行。求其动能、势能、总机械能之比 $K:U:E$，并说明符号含义。
\end{exercise}

\begin{exercise}
某行星绕太阳作椭圆运动，近日点距离为 $r_1$，远日点距离为 $r_2$。利用角动量守恒证明近日点速度与远日点速度之比为 $v_1:v_2=r_2:r_1$。
\end{exercise}

\begin{exercise}
设某行星绕太阳做半径为 $r$ 的圆周运动。仅用牛顿第二定律与万有引力定律，推导开普勒第三定律在圆轨道情形下的表达式。
\end{exercise}

\begin{exercise}
证明：若地球半径增加到原来的 $2$ 倍而质量保持不变，则其第一宇宙速度变为原来的 $1/\sqrt2$ 倍。
\end{exercise}

\begin{exercise}
设外部天体对某星球表面的引力加速度为 $g(r)=GM/r^2$。当距离从 $R$ 改变到 $R+a$ 时，用微分近似求 $\Delta g$，并写出误差来源。
\end{exercise}

\begin{exercise}
某探测器从地球表面竖直发射，初速度为 $v_0$，忽略空气阻力。利用机械能守恒，求其能到达的最大距离 $r_{\max}$（以地心为参考，设 $v_0<v_2$）。
\end{exercise}

\begin{exercise}
在双星系统中，若两星质量相等、中心距为 $L$，求它们各自绕质心运动的线速度和周期。
\end{exercise}

\begin{exercise}
阅读本章关于潮汐力的推导，解释为何潮汐效应与距离的三次方成反比，而不是二次方成反比。
\end{exercise}
