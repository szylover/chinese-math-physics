...
\begin{gaokao}[title={高考题的微积分视角：题一　含电表的分压电路}]
\textbf{题目：} 电源电动势为 $12\,\text{V}$，内阻不计。定值电阻 $R_1=4\,\Omega$ 与滑动变阻器 $R_2$ 串联，电流表测回路电流。若某时刻滑动变阻器接入阻值为 $2\,\Omega$，求回路电流与 $R_1$ 两端电压；若将 $R_2$ 缓慢增大到 $8\,\Omega$，说明电流表和电压表示数如何变化。
...
对其求导：
\[
\dv{I}{R_2}=-\frac{12}{(4+R_2)^2}<0.
\]
因此随着滑动变阻器阻值增大，电流单调减小。又
\[
U_1(R_2)=R_1I=\frac{48}{4+R_2},
\]
同样有
\[
\dv{U_1}{R_2}<0,
\]
故电压表示数也单调减小。这里“微积分视角”的意义在于：通过导数可以直接判断单调性，而无需逐点比较。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：题二　RC 充电到某一电压所需时间}]
\textbf{题目：} 某电容器经电阻 $R$ 与电源 $\varepsilon$ 串联充电。若要求电容器两端电压达到电源电压的 $90\%$，求所需时间。

\textbf{解析：}
RC 充电时
\[
U_C(t)=\frac{q(t)}{C}=\varepsilon\left(1-\mathrm{e}^{-t/(RC)}\right).
\]
令
\[
U_C(t)=0.9\varepsilon,
\]
则有
\[
1-\mathrm{e}^{-t/(RC)}=0.9,
...
两边取自然对数得
\[
-\frac{t}{RC}=\ln 0.1=-\ln 10,
\]
故
\[
t=RC\ln 10.
\]
这类题若只背“几个时间常数”，只能得到近似结论；用微分方程的精确解就能直接求得准确时间。
\end{gaokao}

\begin{gaokao}[title={高考题的微积分视角：题三　变电流下的电热计算}]
\textbf{题目：} 某导体电阻恒为 $R=3\,\Omega$，通过它的电流满足
\[
I(t)=2+t \qquad (0\le t\le 2\,\text{s}).
\]
求这 $2\,\text{s}$ 内导体产生的热量。

\textbf{解析：}
不能直接套用 $Q=I^2Rt$，因为这里电流随时间变化。应采用积分形式：
\[
Q=\int_0^2 I^2(t)R\,\dd t
=3\int_0^2 (2+t)^2\,\dd t.
\]
展开得
\[
Q=3\int_0^2 (t^2+4t+4)\,\dd t
=3\left[\frac{t^3}{3}+2t^2+4t\right]_0^2.
\]
故
\[
Q=3\left(\frac{8}{3}+8+8\right)=56\,\text{J}.
\]
这道题揭示了一个常被忽视的事实：焦耳定律的本质形式其实是积分，而不是仅限于恒定电流时的乘法公式。
\end{gaokao}

\section{习题}

\begin{exercise}
已知某导线中通过横截面的电量满足 $Q(t)=5t^2-2t+1$（单位：C）。求 $t=3\,\text{s}$ 时的瞬时电流，以及从 $0$ 到 $3\,\text{s}$ 的平均电流。
\end{exercise}

\begin{exercise}
某导体中自由载流子数密度为 $n$，每个载流子带电量为 $q$，漂移速度大小为 $v_d$，导体截面积为 $S$。证明总电流大小为 $I=nqv_dS$。
\end{exercise}

\begin{exercise}
一根均匀电阻丝的长度增大为原来的 $2$ 倍，体积不变，材料不变。求其电阻变为原来的多少倍，并说明理由。
\end{exercise}

\begin{exercise}
在某节点处，四条支路电流的参考方向都取为流出节点，其中三条支路电流分别为 $1\,\text{A}$、$-2\,\text{A}$、$3\,\text{A}$。求第四条支路电流。
\end{exercise}

\begin{exercise}
某单回路电路由电源 $\varepsilon=9\,\text{V}$ 和两个串联电阻 $R_1=1\,\Omega$、$R_2=2\,\Omega$ 组成。求回路电流以及两个电阻上的功率。
\end{exercise}

\begin{exercise}
某 RC 充电电路中，$R=50\,\text{k}\Omega$，$C=20\,\mu\text{F}$，$\varepsilon=6\,\text{V}$。求时间常数、稳定后电容器所带电荷量以及 $t=\tau$ 时的电流。
\end{exercise}

\begin{exercise}
某电容器初始电荷量为 $Q_0$，通过电阻 $R$ 放电。求放电到原来电荷量的 $\frac{1}{4}$ 时所需时间。
\end{exercise}

\begin{exercise}
某 RL 电路中，$L=1.2\,\text{H}$，$R=6\,\Omega$，$\varepsilon=12\,\text{V}$。求稳态电流与时间常数，并写出电流随时间的函数关系。
\end{exercise}

\begin{exercise}
某电阻上的电流满足 $I(t)=I_0\mathrm{e}^{-\alpha t}$。求从 $0$ 到 $T$ 时间内产生的热量，并讨论当 $T\to\infty$ 时的极限值。
\end{exercise}

\begin{exercise}
证明：在 RC 充电过程中，任意时刻电阻上的功率为
\[
P_R(t)=\frac{\varepsilon^2}{R}\mathrm{e}^{-2t/(RC)}.
\]
并据此积分验证整个过程中电阻耗散的总热量为 $\frac{1}{2}C\varepsilon^2$。
\end{exercise}
