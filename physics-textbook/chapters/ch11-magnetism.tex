...
本节用三个经典模型展示“电流元积分”的威力：长直导线、圆环以及长螺线管。它们既是电磁学中的基本结果，也是高考和竞赛题中最常见的磁场背景。
...
下面选取三类典型高考题型：粒子偏转、速度选择和导线受力。题目的物理背景属于高中范围，但我们用微积分视角把公式之间的联系讲得更清楚。

\begin{gaokao}
\textbf{题目一：粒子垂直进入匀强磁场}

一带正电粒子从 $A$ 点以速度 $v$ 垂直射入宽度足够大的匀强磁场，磁场方向垂直纸面向里。已知粒子从磁场边界另一点 $B$ 射出，弧长对应的圆心角为 $120^\circ$。求粒子的轨道半径和在磁场中的运动时间。

\textbf{微积分视角分析：}
磁场不做功，因此速率保持为 $v$。粒子做匀速圆周运动，满足
\[
  qvB=\frac{mv^2}{r},
  \qquad
  r=\frac{mv}{qB}.
\]
这一步来自微分方程 $m\dv{\vb{v}}{t}=q\vb{v}\times\vb{B}$ 在速率恒定情形下的曲率解释：加速度大小为 $v^2/r$。

圆周运动角速度为
\[
  \omega=\frac{qB}{m},
\]
因此转过 $120^\circ=2\pi/3$ 所需时间为
\[
  t=\frac{\Delta\varphi}{\omega}=\frac{2\pi/3}{qB/m}=\frac{2\pi m}{3qB}.
\]

\textbf{点评：} 高考常把几何图形作为主线，但真正决定半径和时间的，仍然是“洛伦兹力提供曲率”与“角速度恒定”两个微积分结论。
\end{gaokao}

\begin{gaokao}
\textbf{题目二：速度选择器的条件}

某装置中，电场 $\vb{E}$ 与磁场 $\vb{B}$ 相互垂直，带电粒子沿与二者都垂直的方向射入。要使粒子沿直线匀速通过而不偏转，粒子的速度应满足什么条件？若速度偏大，粒子将向哪侧偏转？

\textbf{微积分视角分析：}
虽然本章重点是磁场，但真正的动力学方程是完整的洛伦兹力：
\[
  m\dv{\vb{v}}{t}=q(\vb{E}+\vb{v}\times\vb{B}).
\]
要想做直线匀速运动，必须有合力恒为零，即
\[
  qE=qvB,
\]
从而
\[
  v=\frac{E}{B}.
\]
这就是速度选择器。若 $v>E/B$，则磁力大于电场力，粒子向磁力方向偏转；若 $v<E/B$，则向电场力方向偏转。

\textbf{点评：} 高中解法常直接“令电场力等于洛伦兹力”，而微积分视角强调的是：直线匀速其实就是微分方程右端为零的平衡解。
\end{gaokao}

\begin{gaokao}
\textbf{题目三：平行导线平衡问题}

两条平行长直导线相距 $d$，上方导线固定并通电流 $I_1$，下方导线质量线密度为 $\lambda$，通同向电流 $I_2$。若要使下方导线恰好悬浮，求两电流应满足的关系。
...
于是下方导线单位长度所受向上的安培力为
\[
  \frac{F}{L}=I_2B=\frac{\mu_0 I_1I_2}{2\pi d}.
\]
悬浮条件是单位长度安培力等于单位长度重力：
\[
  \frac{\mu_0 I_1I_2}{2\pi d}=\lambda g.
\]
故
\[
  I_1I_2=\frac{2\pi d\lambda g}{\mu_0}.
\]

\textbf{点评：} 这类题表面是平衡问题，实质上先要用长直导线磁场公式把“场”算出来，再用安培力把“场”转化为“力”。整个过程体现了“电流 $\to$ 磁场 $\to$ 力”的链条。
\end{gaokao}
...
\begin{exercise}
证明：若粒子只受磁场力作用，则其速率恒定；进一步说明其动量大小也恒定，但动量方向随时间改变。
\end{exercise}

\begin{exercise}
一段有限长直导线与点 $P$ 的垂直距离为 $a$，两端点对 $P$ 的张角分别为 $\theta_1$、$\theta_2$。请从毕奥--萨伐尔定律出发，完整推导
\[
  B=\frac{\mu_0 I}{4\pi a}(\sin\theta_1+\sin\theta_2).
\]
\end{exercise}
...
\begin{exercise}
半径为 $R$ 的半圆形导线通电流 $I$，求圆心处磁感应强度。若在半圆两端再接两段沿直径方向的导线，它们对圆心处磁场有无贡献？请说明理由。
\end{exercise}

\begin{exercise}
利用圆环轴线磁场公式，求双匝同轴圆环（半径均为 $R$、电流同向）在圆心处的磁感应强度；若电流反向，结果如何变化？
\end{exercise}

\begin{exercise}
设一长直螺线管有限长为 $L$、半径为 $R$、总匝数为 $N$。仿照本章的圆环叠加方法，写出其轴线上任意点磁感应强度的积分表达式，并讨论当 $L\gg R$ 时中心点结果如何近似化简。
\end{exercise}

\begin{exercise}
一质量为 $m$、电荷量为 $q$ 的粒子以速度 $v$ 垂直射入匀强磁场，经过半圆后从另一侧射出。求它在磁场中的运动时间，并说明这个时间与速度 $v$ 是否有关。
\end{exercise}

\begin{exercise}
带电粒子以速度 $v$ 与匀强磁场成角 $\theta$ 射入，已知螺距为 $h$。试用 $m,q,B,h$ 表示其速度平行分量 $v_\parallel$；再结合半径公式写出总速度 $v$ 与螺旋半径 $r$ 的关系。
\end{exercise}

\begin{exercise}
两条相距 $d$ 的平行直导线通有反向电流 $I$。求每单位长度相互作用力大小，并判断是吸引还是排斥。若把距离减半，作用力如何变化？
\end{exercise}

\begin{exercise}
一段长度为 $L$ 的导线置于非匀强磁场中，磁场方向处处相同但大小沿导线方向满足 $B(x)=B_0+kx$。若导线与磁场垂直、通电流 $I$，请用积分求安培力大小。
\end{exercise}

\begin{exercise}
综合题：一带电粒子先后穿过速度选择器和匀强磁场偏转区。已知选择器中的电场强度为 $E$、磁感应强度为 $B_1$，偏转区磁感应强度为 $B_2$。粒子恰能在选择器中直线通过。求其进入偏转区后的轨道半径，并讨论该半径如何随 $E$、$B_1$、$B_2$ 变化。
\end{exercise}
