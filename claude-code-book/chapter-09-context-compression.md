...
**工作原理**：
- 每个工具可以定义 `maxResultSizeChars` 限制
- 超出限制的工具结果被替换为摘要
- 替换决策通过 `contentReplacementState` 持久化
- 支持会话恢复时重建替换状态

**与微压缩的关系**：工具结果预算在微压缩之前运行。由于缓存微压缩完全通过 `tool_use_id` 操作（不检查内容），内容替换对它是透明的，两者可以干净地组合。
...
- 标识旧的对话片段
- 移除不再需要的旧片段
- 通过 `snipReplay` 回调处理 snip 边界
- 释放的 Token 数量传递给自动压缩阈值计算
...
缓存微压缩是一种优化：使用 Anthropic API 的缓存编辑功能，延迟边界消息直到 API 响应后才发送（使用实际的 `cache_deleted_input_tokens` 而非估计值）。
...
**设计特点**：
- 上下文折叠在自动压缩之前运行
- 如果折叠使 Token 数降到自动压缩阈值以下，自动压缩就不会触发
- 折叠视图是**读时投影**，不修改 REPL 的完整历史
- 摘要消息存储在折叠存储中，而非 REPL 消息数组
- `projectView()` 在每次查询入口重放提交日志
...
自动压缩是最后的安全网：
- 当 Token 使用接近上下文窗口限制时自动触发
- 使用 LLM 生成整个对话的摘要
- 替换原有消息历史为摘要 + 最近的消息
- 通过 `/compact` 命令也可以手动触发
...
Token 计数使用两种策略：
1. **精确计数**：从 API 响应的 `usage` 字段获取
2. **估算**：在 API 调用之前，使用估算方法预测 Token 数量
...
Token 预算系统（`TOKEN_BUDGET` Feature Flag）追踪每个回合的 Token 消耗：
...
任务预算通过 API 的 `output_config.task_budget` 参数控制。压缩后需要重新计算剩余预算：
...
当模型输出被 `max_output_tokens` 截断时：
1. 错误消息被"扣留"，不发送给 SDK 调用方
2. 自动向 API 发送继续请求
3. 最多重试 3 次
4. 只有确认恢复失败后才将错误报告给调用方
...
**下一章**：[多智能体协调 (Coordinator Mode)](./chapter-10-coordinator.md)
