...
| `plan` | 计划模式——只允许只读操作，不执行修改 |
| `bypassPermissions` | 绕过权限——所有操作自动批准（危险） |
| `auto` | 自动模式——使用分类器自动判断是否安全 |
...
自动模式使用**对话分类器**（Transcript Classifier）分析工具调用的安全性。分类器在权限对话框显示之前运行，可以自动批准被认为安全的操作。

## 8.3 权限规则

权限规则按来源分组（`ToolPermissionRulesBySource`），支持三种决策：
...
后台代理（如通过 `AgentTool` 生成的子代理）无法显示 UI，因此权限提示被自动拒绝。这要求后台代理只执行已预先授权的操作。
...
某些权限在自动模式下被主动剥离——即使用户配置了 `alwaysAllow`，某些高风险操作在自动模式下仍需要确认。
...
权限系统体现了纵深防御原则：
1. **工具列表过滤**：拒绝的工具不暴露给模型
2. **调用时检查**：每次调用都经过权限验证
3. **沙箱执行**：即使允许，也在受限环境中执行
4. **追踪与审计**：所有决策都被记录

### 渐进式信任

从 `plan`（只读）到 `default`（需确认）到 `auto`（自动判断）到 `bypassPermissions`（全部允许），提供了渐进式的信任层级。

---

**下一章**：[上下文压缩与 Token 管理](./chapter-09-context-compression.md)
