# 第四章：核心查询引擎 QueryEngine

## 4.1 QueryEngine 概述

`QueryEngine` 是 Claude Code 的核心类，定义在 `src/QueryEngine.ts` 中（约 46,000 行）。它拥有整个对话生命周期的状态，管理与 Anthropic API 的通信、工具调用循环、流式响应处理等。
...
`submitMessage` 是一个 **AsyncGenerator**——它通过 `yield` 逐步产出流式事件，使调用方能实时获取 API 响应的每一部分。
...
- **`adaptive`**：模型自行决定是否需要深度思考
- **`enabled`**：始终启用扩展思考
- **`disabled`**：禁用扩展思考

## 4.3 查询循环 (query.ts)

`query.ts` 实现了与 API 交互的核心循环——这是 Claude Code 最复杂的控制流之一。
...
查询循环中的消息在发送到 API 之前，会经过一个**多层压缩管线**：
...
1. **`readFileState`**：文件状态 LRU 缓存。避免重复读取同一文件。
2. **`setAppStateForTasks`**：允许嵌套子代理更新根状态存储，即使子代理的 `setAppState` 是空操作。
3. **`contentReplacementState`**：工具结果预算的替换状态，跨轮次持久化。
4. **`renderedSystemPrompt`**：父代理的渲染系统提示的冻结快照，用于 fork 子代理共享缓存。
...
当 `TOKEN_BUDGET` Feature Flag 启用时，查询循环会追踪 Token 消耗并与预算比较。

**任务预算 (Task Budget)**：通过 API 的 `output_config.task_budget` 参数设置整个代理回合的预算。每次压缩后需要重新计算剩余预算。

## 4.7 设计启示

### AsyncGenerator 模式

使用 `async function*` 实现流式数据处理是一个优雅的设计。调用方可以通过 `for await...of` 消费流，也可以使用 `.next()` 精确控制步进。
...
对 `max_output_tokens` 的自动恢复展示了一个重要原则：**AI 应用应该对 API 的边界条件有鲁棒的处理策略**，而不是简单地向用户抛出错误。

---

**下一章**：[上下文处理与 System Prompt](./chapter-05-context.md)
